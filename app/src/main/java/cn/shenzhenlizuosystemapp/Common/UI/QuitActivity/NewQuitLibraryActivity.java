...
import cn.shenzhenlizuosystemapp.Common.Adapter.QuitAdapter.ScanResult_QuitRvAdapter;
import cn.shenzhenlizuosystemapp.Common.Adapter.QuitAdapter.ScanTask_QuitRvAdapter;
import cn.shenzhenlizuosystemapp.Common.AsyncGetData.QuitTask.QuitBarCodeCheckTask;
import cn.shenzhenlizuosystemapp.Common.AsyncGetData.QuitTask.QuitBillCreateTask;
import cn.shenzhenlizuosystemapp.Common.AsyncGetData.QuitTask.QuitBodyLockTask;
...
import cn.shenzhenlizuosystemapp.Common.DataAnalysis.QuitDataAnalysis.QuitLibraryDetail;
import cn.shenzhenlizuosystemapp.Common.DataAnalysis.QuitDataAnalysis.QuitSubBodyBean;
import cn.shenzhenlizuosystemapp.Common.DataAnalysis.QuitDataAnalysis.QuitTaskRvData;
import cn.shenzhenlizuosystemapp.Common.DataAnalysis.QuitDataAnalysis.QuitMaterialModeBean;
import cn.shenzhenlizuosystemapp.Common.DataAnalysis.QuitDataAnalysis.QuitStockBean;
import cn.shenzhenlizuosystemapp.Common.DataAnalysis.QuitDataAnalysis.QuitStock_Return;
...
import cn.shenzhenlizuosystemapp.Common.Port.QuitPort.QuitBillCreate;
import cn.shenzhenlizuosystemapp.Common.Port.LockResultPort;
import cn.shenzhenlizuosystemapp.Common.SpinnerAdapter.QuitSpinnerAdapter.QuitStockAdapter;
import cn.shenzhenlizuosystemapp.Common.SpinnerAdapter.QuitSpinnerAdapter.QuitMaterialModeAdapter;
...
import cn.shenzhenlizuosystemapp.Common.WebBean.QuitBean.QuitLibraryAllInfo;
import cn.shenzhenlizuosystemapp.Common.Xml.QuitXml.QuitAnalysisMaterialModeXml;
...
import cn.shenzhenlizuosystemapp.Common.Xml.QuitXml.QuitLibraryXmlAnalysis;
import cn.shenzhenlizuosystemapp.Common.Xml.QuitXml.QuitStockXmlAnalysis;
import cn.shenzhenlizuosystemapp.Common.Xml.QuitXml.QuitTagModeAnalysis;
...
public class NewQuitLibraryActivity extends BaseActivity {
        ...
        editSumPort = new EditSumPort() {
            ...
            public void OnEnSure(String Sum) {
                if (Tools.StringOfFloat(Sum) <= Tools.StringOfFloat(quitTaskRvDataList.get(RV_ScanInfoTableIndex).getNoSend()) && Tools.StringOfFloat(Sum) > 0) {
                    EditSumDialog.getSingleton(NewQuitLibraryActivity.this).Dismiss();
                    SubmitData(Sum);
                    InputMethodManager inputManager = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
                    inputManager.toggleSoftInput(0, 0);
                } else {
                    EditSumDialog.getSingleton(NewQuitLibraryActivity.this).ShowErrorInfo("输入数据有误");
                }
            }
        };
    ...
    private void InitClick() {
        ...
        SoftKeyBoardListener.setListener(NewQuitLibraryActivity.this, new SoftKeyBoardListener.OnSoftKeyBoardChangeListener() {
            @Override
    ...
    private void InitScanRecycler() {
        ...
        if (Tools.IsObjectNull(scanTask_quit_rvAdapter)) {
            ScanTaskItemClick(0);
        ...
        if (Integer.parseInt(quitTaskRvDataList.get(position).getFAuxQty().split("\\.")[0]) <= Integer.parseInt(quitTaskRvDataList.get(position).getFThisAuxQty().split("\\.")[0]) +
            ...
            if (!Is_QuitNumber_Mode) {
                if (scanTask_quit_rvAdapter.getselection() != position) {
                    ...
                    if (Tools.IsObjectNull(scanResult_Quit_rvAdapter)) {
                ...
                if (!TextUtils.isEmpty(QuitSum) && Tools.StringOfFloat(QuitSum) > 0) {
                    if (CheckGuid(CheckFGuid, FBarcodeLib)) {
                        ...
                        if (Tools.StringOfFloat(quitTaskRvDataList.get(RV_ScanInfoTableIndex).getNoSend()) >= Tools.StringOfFloat(QuitSum)) {
                            ...
                            if (Tools.StringOfFloat(quitTaskRvDataList.get(RV_ScanInfoTableIndex).getNoSend()) == 0) {
                                quitTaskRvDataList.remove(RV_ScanInfoTableIndex);
                                if (quitTaskRvDataList.size() > RV_ScanInfoTableIndex) {
...
, stockBeanList.get(SpQuitHouseSpaceIndex).getFGuid(), QuitSubmitList, webService, myProgressDialog, inputBillCreate);
    ...
    private void ShowEditSumDialog(String Sum) {
        EditSumDialog.getSingleton(NewQuitLibraryActivity.this).Show(MContect, quitTaskRvDataList.get(RV_ScanInfoTableIndex).getFMaterial_Code(), editSumPort, new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                EditSumDialog.getSingleton(NewQuitLibraryActivity.this).Dismiss();
                CleanData();
                ...
                Batch,quitTaskRvDataList.get(RV_ScanInfoTableIndex).getFMaterial_Name()+quitTaskRvDataList.get(RV_ScanInfoTableIndex).getFModel());
    }
