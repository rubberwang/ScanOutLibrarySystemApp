...
public class StocksCallXml {

    private volatile static StocksCallXml stocksCallXml;

    public static StocksCallXml getSingleton() {
        if (stocksCallXml == null) {
            synchronized (StocksCallXml.class) {
                if (stocksCallXml == null) {
                    stocksCallXml = new StocksCallXml();
                }
            }
        }
        return stocksCallXml;
    }

    public List GetStocksCallXml(InputStream stream) throws SAXException, IOException, ParserConfigurationException {
